...
(() => {
  ...
  var Card = class {
    ...
    async handleRequest(endpoint, data) {
      return this.handler.request(`card/${endpoint}`, { id: this.id, data });
    }
  };
  ...
  var Note = class {
    ...
    async handleRequest(endpoint, data) {
      return this.handler.request(`note/${endpoint}`, { id: this.id, data });
    }
  };
  ...
  var Deck = class {
    ...
    async handleRequest(endpoint, data) {
      return this.handler.request(`deck/${endpoint}`, { id: this.id, data });
    }
  };
  ...
  var Android = class {
    constructor(handler) {
       ...
       */
      this.isNetworkMetered = () => this.handleRequest(androidEndpoints.IS_NETWORK_METERED);
    }
    async handleRequest(endpoint, data) {
  ...
  var StudyScreen = class {
    constructor(contract) {
      ...
      this.deck = this.createProxy(this.deckPromise);
    }
    get cardPromise() {
      if (!this._cardPromise) {
        this._cardPromise = (async () => {
          const cardId = await this.request("studyScreen/getCurrentCardId");
          return new Card(this, cardId.value);
        })();
      }
    ...
    get notePromise() {
      if (!this._notePromise) {
        this._notePromise = (async () => {
          const card = await this.cardPromise;
          const nid = await card.getNid();
          return new Note(this, nid.value);
        })();
      }
      return this._notePromise;
    }
    get deckPromise() {
      if (!this._deckPromise) {
        this._deckPromise = (async () => {
          const card = await this.cardPromise;
          const did = await card.getDid();
          return new Deck(this, did.value);
        })();
      }
      return this._deckPromise;
    }
    createProxy(promise) {
      return new Proxy({}, {
        get: (_target, prop) => {
