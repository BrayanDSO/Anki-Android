...
import com.ichi2.anki.LoginActivity.Companion.HIDE_REGISTER_EXTRA
...
class MyAccount : Fragment() {
    ...
    override fun onCreateView(
        ...
        if (!isLoggedIn()) {
            ...
            if (arguments?.getBoolean(HIDE_REGISTER_EXTRA, false) == true) {
    ...
    private fun logout() {
        launchCatchingTask {
            ...
            getInstance(requireContext()).reset()
            recreate()
        }
    }

    /**
     * Destroy the fragment and create it again on [R.id.fragment_container].
     * Useful to reload the view after logging in/out
     *
     * Also notifies that the login status has changed
     * with a fragment result of key [SyncSettingsFragment.HAS_LOGIN_STATUS_CHANGED_KEY]
     */
    fun recreate() {
        parentFragmentManager.popBackStack()
        parentFragmentManager.commit {
            remove(this@MyAccount)
            replace(R.id.fragment_container, MyAccount())
            addToBackStack(null)
        }
        setFragmentResult(SyncSettingsFragment.HAS_LOGIN_STATUS_CHANGED_KEY, bundleOf(SyncSettingsFragment.HAS_LOGIN_STATUS_CHANGED_KEY to true))
    }
