...
class WhiteboardView : View {
     ...
     */
    private var isStylusButtonOverriding = false
    ...
    private fun updateStylusButtonState(event: MotionEvent) {
        val isButtonPressed = event.hasStylusButtonPressed()
        if (isButtonPressed) {
            if (!isStylusButtonOverriding && activeTool is WhiteboardTool.Brush) {
                isStylusButtonOverriding = true
                onStylusButtonStateChanged?.invoke(true)
            }
        } else {
            isStylusButtonOverriding = false
            onStylusButtonStateChanged?.invoke(false)
        }
    }
