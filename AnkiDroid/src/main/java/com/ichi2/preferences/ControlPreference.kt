...
open class ControlPreference :
    ...
    open fun warnIfUsed(
        ...
        val bindingPreference = getOtherPreferenceAssignedTo(binding) ?: return false
    ...
    private fun addBinding(binding: Binding) {
        val mappableBinding = MappableBinding(binding)
        getOtherPreferenceAssignedTo(binding)?.removeMappableBinding(mappableBinding)

        val bindings = getMappableBindings().toMutableList()
        bindings.add(mappableBinding)
        value = bindings.toPreferenceString()
    }
     ...
     */
    protected fun getOtherPreferenceAssignedTo(binding: Binding): ControlPreference? {
        for (pref in preferenceManager.preferenceScreen.allPreferences()) {
            ...
            if (binding in bindings) {
                return if (pref == this) null else pref
            }
        }
        return null
    }
